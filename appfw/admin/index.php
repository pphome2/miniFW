...
if ($SYS_OK){
  # app betöltése
  $APP_OK=false;
  $fn="../".$fwcfg->{'FW_CONTENT_DIR'}."/".$fwcfg->{'FW_APP_ADMIN_PHP'};
  if (file_exists($fn)){
    $APP_OK=include($fn);
  }
  ...
  if ($APP_OK){
    ...
    $fwcfg->{'FW_TEMPLATE_ADMIN_PHP'}=$fwapp->{'APP_TEMPLATE'}."/".$fwcfg->{'FW_TEMPLATE_ADMIN_PHP'};
  }
  # template betöltése
  $TEMP_OK=false;
  $fn="../".$fwcfg->{'FW_TEMPLATE_DIR'}."/".$fwcfg->{'FW_TEMPLATE_ADMIN_PHP'};
  if (file_exists($fn)){
    $TEMP_OK=include($fn);
    if ($TEMP_OK){
